 ...
 */
package com.androidpdfviewdowload.createpdf.lowagie.examples.fonts;
...
public class EncodingFont {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// step 2
			PdfWriter.getInstance(document, new FileOutputStream(android.os.Environment.getExternalStorageDirectory() + java.io.File.separator + "droidtext" + java.io.File.separator + "encodingfont.pdf"));
			// step 3
			document.open();
			// step 4
			...
			for (int z = 0; z < all.length; ++z) {
				String file = all[z];
				document.add(new Paragraph("Unicode equivalence for the font \"" + file + "\" with the encoding \""
						+ file + "\"\n"));
				char tb[];
				if (z == 0)
					tb = SYMBOL_TABLE;
