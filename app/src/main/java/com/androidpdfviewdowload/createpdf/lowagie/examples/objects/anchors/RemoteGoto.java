...
package com.androidpdfviewdowload.createpdf.lowagie.examples.objects.anchors;
...
public class RemoteGoto {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// step 2:
			PdfWriter writerA = PdfWriter.getInstance(document, new FileOutputStream(android.os.Environment.getExternalStorageDirectory() + java.io.File.separator + "droidtext" + java.io.File.separator + "DocumentA.pdf"));
			PdfWriter writerB = PdfWriter.getInstance(document, new FileOutputStream(android.os.Environment.getExternalStorageDirectory() + java.io.File.separator + "droidtext" + java.io.File.separator + "DocumentB.pdf"));
			...
			Paragraph p1 = new Paragraph("You can turn a Chunk into an ", FontFactory
					.getFont(FontFactory.HELVETICA, 12));
			p1.add(new Chunk("anchor", FontFactory.getFont(FontFactory.HELVETICA, 12, Font.UNDERLINE, new Color(0, 0,
					255))).setAnchor(new URL("http://www.lowagie.com/iText/")));
