...
public class Patterns {
	...
	public static void main(String[] args) {
		...
		try {
			...
			PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(android.os.Environment.getExternalStorageDirectory() + java.io.File.separator + "droidtext" + java.io.File.separator + "patterns.pdf"));
			...
			// step 5: we create some PdfPatternPainter instances for drawing
			// path, text, or placing image
			...
			// See if we can apply the pattern color to chunk, phrase or
			// paragraph
			...
			document.add(new Paragraph(text, FontFactory.getFont(FontFactory.HELVETICA, 60, Font.BOLD, new GrayColor(
					0.3f))));
			...
			// draw some characters.
			// Note: if the image fills up the pattern, there's no need to draw
			// text twice
