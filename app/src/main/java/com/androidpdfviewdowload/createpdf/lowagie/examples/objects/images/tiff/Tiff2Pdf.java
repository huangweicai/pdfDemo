...
public class Tiff2Pdf {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			try {
				PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(android.os.Environment.getExternalStorageDirectory() + java.io.File.separator + "droidtext" + java.io.File.separator + pdf_file));
				int pages = 0;
				document.open();
...
//					ra = new RandomAccessFileOrArray(tiff_file);
					ra = new RandomAccessFileOrArray(PdfTestRunner.getActivity().getResources().openRawResource(R.raw.tif_12));
