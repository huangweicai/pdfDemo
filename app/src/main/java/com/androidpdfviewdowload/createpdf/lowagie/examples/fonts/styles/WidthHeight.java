...
public class WidthHeight {
	...
	public static void main(String[] args) {
		...
		try {
			...
			PdfWriter.getInstance(document, new FileOutputStream(android.os.Environment.getExternalStorageDirectory() + java.io.File.separator + "droidtext" + java.io.File.separator + "widthheight.pdf"));
...
//			BaseFont bfComic = BaseFont
//					.createFont("FreeSerifBoldItalic.ttf", BaseFont.WINANSI, BaseFont.EMBEDDED);
			InputStream inputStream = PdfTestRunner.getActivity().getResources().openRawResource(R.raw.freesans);
			byte[] buffer = new byte[inputStream.available()];
			inputStream.read(buffer);
			BaseFont bfComic = BaseFont.createFont("freeserifbolditalic.ttf", BaseFont.IDENTITY_H, true, false, buffer, null);
			...
			document.add(new Paragraph("height: "
					+ (bfComic.getAscentPoint(text1 + text2 + text3, 12) - bfComic.getDescentPoint(text1 + text2
							+ text3, 12))));
