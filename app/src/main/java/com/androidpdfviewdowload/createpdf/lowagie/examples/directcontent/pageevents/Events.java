 ...
 */
package com.androidpdfviewdowload.createpdf.lowagie.examples.directcontent.pageevents;
...
public class Events {
	...
	class MyPageEvents extends PdfPageEventHelper {
		 ...
		 * speakers.
		 * 
		 * @see PdfPageEventHelper#onGenericTag(PdfWriter,
		 *      Document, Rectangle,
		 *      String)
		 */
		 ...
		 * 
		 * @see PdfPageEventHelper#onChapter(PdfWriter,
		 *      Document, float, Paragraph)
		 */
	...
	public static void main(String[] args) {
		...
		try {
			...
			PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(android.os.Environment.getExternalStorageDirectory() + java.io.File.separator + "droidtext" + java.io.File.separator + "RomeoJuliet.pdf"));
			...
			parser.parse(PdfTestRunner.getActivity().getResources().openRawResource(R.raw.playromeojuliet), new Events().getXmlHandler(document));
	...
	class RomeoJulietMap extends TagMap {
		...
		public RomeoJulietMap() throws IOException {
			//Can't use FileInputStream directly
			//super(new FileInputStream("tagmapRomeoJuliet.xml"));
			super(PdfTestRunner.getActivity().getResources().openRawResource(R.raw.tagmapromeojuliet));
